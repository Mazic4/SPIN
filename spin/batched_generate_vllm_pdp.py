...
def generate_on_single_gpu(
    ...
    prompts_all = [
        "<s>[INST] <<SYS>>\n" + data[idx][0]['content'] + "\n<</SYS>>\n" + data[idx][1]['content'] + "[/INST]" for idx in range(len(data))
    ]

    prompts_old = [
        "<s>[INST] <<SYS>>\n" + data[idx][0]['content'] + "\n<</SYS>>\n" + data[idx][1]['content'] + "[/INST]" for idx in range(len(data)) for idx in range(len(data))
    ]

    corrects_all = [
        data[idx][2]['content'] for idx in range(len(data))
    ]
    ...
    results = [r.replace("<s>", "").replace("</s>", "").replace("[INST]", "").replace("[/INST]", "").lstrip() for r in results_gathered]
    ...
    for idx in range(len(corrects_all)):
        d = {
            "real": data[idx],
            "generated": data[idx][0:2]+[{"role": "assistant", "content": results[idx]}]
        }
        if split == "test":
