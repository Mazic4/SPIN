...
def generate_on_single_gpu(
    ...
    if frac_len > 0:
        sub_len = frac_len 
        if sub_len*data_frac > len(data):
            _data_frac = int(data_frac%(len(data)//sub_len+1))
            data = data[sub_len*_data_frac:sub_len*(_data_frac+1)]['real']
        else:
            data = data[sub_len*data_frac:sub_len*(data_frac+1)]['real']

    # prompts_all = ['<|user|>\n'.join(data[idx][0:2]) for idx in range(len(data))]
    # prompts_old = ['<|user|>\n'.join(data[idx][0:2]) for idx in range(len(data))]
    # corrects_all = [data[idx][2] for idx in range(len(data))]
    prompts_all = []
    prompts_old = []
    corrects_all = []
    for idx in range(len(data)):
        if len(data[idx]) == 2:
            prompts_all.append(data[idx][0])
            prompts_old.append(data[idx][0])
            corrects_all.append(data[idx][1])
        else:
            prompts_all.append('<|user|>\n'.join(data[idx][0:2]))
            prompts_old.append('<|user|>\n'.join(data[idx][0:2]))
            corrects_all.append(data[idx][2])
    ...
    for idx in range(len(corrects_all)):
        if corrects_all[idx][:1024] == results[idx][:1024]:
            continue
        if "You are an excellent customer service agent who help customers solve their problems" not in prompts_old[idx]:
            if chat_cnt > mrgt_cnt: continue
            else:chat_cnt += 1
        else:
            mrgt_cnt += 1
        d = {
