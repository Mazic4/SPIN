...
version="v2_10"
MAIN_BASE_DIR=$(pwd)/mrgt/train_generated/${version}/
MODEL_BASE_DIR=/maas-vepfs/spin-outputs/ecom/mrgt/${version}/
CONFIG_BASE_DIR="configs/mrgt/${version}/"

mkdir -p ${MODEL_BASE_DIR}
mkdir -p ${MAIN_BASE_DIR}
mkdir -p ${CONFIG_BASE_DIR}

org_data_dir="/maas-vepfs/data/ecom/mrgt/mrgt_mixchat" #mrgt+open_source_chat
out_dir="${MAIN_BASE_DIR}/init"

python spin/reformat_multiround.py --data $org_data_dir --output_dir $out_dir
...
for iter in 0 1 2 3; do
    ...
    if [ "$iter" -eq 0 ]; then
        ...
        model_dir="/maas-vepfs/models/ecom/mrgt/mrgt_mixchat/"
    else
    ...
    python3 spin/batched_generate_vllm_mrgt_mix_v2.py --model $model_dir --input_dir $data_dir --frac_len 20000 --num_data_frac 8 --tp_per_worker 1 --output_dir $output_dir
    python3 spin/batched_generate_vllm_mrgt_mix_v2.py --model $model_dir --input_dir $data_dir --frac_len 500 --num_data_frac 8 --tp_per_worker 1 --split test --output_dir $output_dir
    ...
    bash scripts/write_config_v6.sh $model_dir $data_output_dir": 1.0" $config_path
